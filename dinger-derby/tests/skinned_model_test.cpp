...
int main() {
    ...
    // Peak kick ~ t=0.34 — knee clearly elevated vs rest.
    anim.resetToRest();
    ...
    // Release ~ t=0.66 — wrist should move toward plate (+Z) vs layback.
    anim.applyClipNormalized(windup, 0.58f);
    Vector3 wristLay = anim.jointWorldPosition("Wrist_R");
    ...
    expect(wrist.z > wristLay.z - 0.02f, "release wrist moves forward vs layback");
    expect((wrist - elbow).magnitude() > 0.15f, "arm extended at release");
